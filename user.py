...
if st.button("submit", type="primary",width="stretch"):
    ...
    if required_a<st.session_state["total_hours"]-st.session_state["complete"]:
            st.markdown(f"<h6 style='text-align: center; color: red;'>you need to attend {int(round(required_a))} hours</h6>", unsafe_allow_html=True)
    if required_a>st.session_state["total_hours"]-st.session_state["complete"]:
            st.markdown(f"<h6 style='text-align: center; color: #28a745;'>you cannot achieve your goal of {st.session_state['goal']}</h6>", unsafe_allow_html=True)
        ...
        st.markdown(f"<h6 style='text-align: center; color: #28a745;'> you can bunk {m.floor(st.session_state['attended']/goal - st.session_state['complete'])} hours/{st.session_state['total_hours']} hours</h6>", unsafe_allow_html=True)
